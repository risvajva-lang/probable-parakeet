...
export interface ServerDefinition {
    ...
    enabled: boolean;
    quality: StreamQuality;
    type: 'embed' | 'direct' | 'api' | 'resolver';
    ...
    // URL Building & Resolving
    buildMovieUrl?: (tmdbId: number | string, imdbId?: string) => string;
    buildEpisodeUrl?: (tmdbId: number | string, season: number, episode: number, imdbId?: string) => string;
    buildSearchUrl?: (query: string) => string;
    resolveDirectStream?: (request: MediaRequest, signal?: AbortSignal) => Promise<ServerResult | null>;
}
